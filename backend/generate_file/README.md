...
4. **Archive** data to an OSF project node to establish a permanent record of "prior art".
...
| CSV Generation | `csv_file_generator.py` |
...
* **Logic**: Automatically parses paragraphs from Word and extracts text by line from PDFs.
...
Uploads a CSV file directly to the configured OSF project node.
...
├── csv_file_generator.py  # Advanced CSV parsing logic
├── Procfile               # Deployment configuration
├── test_files/            # Sample files for validation testing
...
* **Validation**: Ensures `size (k)` is not greater than the total number of items.
...
* **Timezone**: Timestamps are localized to `America/Toronto` (EST) for legal logging consistency.
* **CSV Integrity**: The `is_csv()` check ensures the first row has at least 2 columns to verify data quality.
